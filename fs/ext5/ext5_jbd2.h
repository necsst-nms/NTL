...
#define EXT5_INDEX_EXTRA_TRANS_BLOCKS	8

#ifdef CONFIG_QUOTA
/* Amount of blocks needed for quota update - we know that the structure was
 * allocated so we need to update only data block */
#define EXT5_QUOTA_TRANS_BLOCKS(sb) ((test_opt(sb, QUOTA) ||\
		EXT5_HAS_RO_COMPAT_FEATURE(sb, EXT5_FEATURE_RO_COMPAT_QUOTA)) ?\
		1 : 0)
/* Amount of blocks needed for quota insert/delete - we do some block writes
 * but inode, sb and group updates are done only once */
#define EXT5_QUOTA_INIT_BLOCKS(sb) ((test_opt(sb, QUOTA) ||\
		EXT5_HAS_RO_COMPAT_FEATURE(sb, EXT5_FEATURE_RO_COMPAT_QUOTA)) ?\
		(DQUOT_INIT_ALLOC*(EXT5_SINGLEDATA_TRANS_BLOCKS(sb)-3)\
		 +3+DQUOT_INIT_REWRITE) : 0)

#define EXT5_QUOTA_DEL_BLOCKS(sb) ((test_opt(sb, QUOTA) ||\
		EXT5_HAS_RO_COMPAT_FEATURE(sb, EXT5_FEATURE_RO_COMPAT_QUOTA)) ?\
...
#endif
#define EXT5_MAXQUOTAS_TRANS_BLOCKS(sb) (MAXQUOTAS*EXT5_QUOTA_TRANS_BLOCKS(sb))
#define EXT5_MAXQUOTAS_INIT_BLOCKS(sb) (MAXQUOTAS*EXT5_QUOTA_INIT_BLOCKS(sb))
#define EXT5_MAXQUOTAS_DEL_BLOCKS(sb) (MAXQUOTAS*EXT5_QUOTA_DEL_BLOCKS(sb))

static inline int ext5_jbd2_credits_xattr(struct inode *inode)
{
	int credits = EXT5_DATA_TRANS_BLOCKS(inode->i_sb);
	...
	if (ext5_has_inline_data(inode))
		credits += ext5_writepage_trans_blocks(inode) + 1;
	return credits;
}
...
#define EXT5_HT_MAX             11

/**
 *   struct ext5_journal_cb_entry - Base structure for callback information.
 *
 *   This struct is a 'seed' structure for a using with your own callback
 *   structs. If you are using callbacks you must allocate one of these
 *   or another struct of your own definition which has this struct
 *   as it's first element and pass it to ext5_journal_callback_add().
 */
struct ext5_journal_cb_entry {
	/* list information for other callbacks attached to the same handle */
	struct list_head jce_list;

	/*  Function to call with this callback structure */
	void (*jce_func)(struct super_block *sb,
			 struct ext5_journal_cb_entry *jce, int error);

	/* user data goes here */
};

/**
 * ext5_journal_callback_add: add a function to call after transaction commit
 ...
 */
static inline void ext5_journal_callback_add(handle_t *handle,
			...
			struct ext5_journal_cb_entry *jce)
{
	struct ext5_sb_info *sbi =
			EXT5_SB(handle->h_transaction->t_journal->j_private);
...
/**
 * ext5_journal_callback_del: delete a registered callback
 * @handle: active journal transaction handle on which callback was registered
 * @jce: registered journal callback entry to unregister
 * Return true if object was sucessfully removed
 */
static inline bool ext5_journal_callback_try_del(handle_t *handle,
					     struct ext5_journal_cb_entry *jce)
{
	bool deleted;
	struct ext5_sb_info *sbi =
...
#define ext5_journal_get_write_access(handle, bh) \
	__ext5_journal_get_write_access(__func__, __LINE__, (handle), (bh))
#define ext5_forget(handle, is_metadata, inode, bh, block_nr) \
	__ext5_forget(__func__, __LINE__, (handle), (is_metadata), (inode), \
		      (bh), (block_nr))
#define ext5_journal_get_create_access(handle, bh) \
	__ext5_journal_get_create_access(__func__, __LINE__, (handle), (bh))
#define ext5_handle_dirty_metadata(handle, inode, bh) \
	__ext5_handle_dirty_metadata(__func__, __LINE__, (handle), (inode), \
				     (bh))
#define ext5_handle_dirty_super(handle, sb) \
	__ext5_handle_dirty_super(__func__, __LINE__, (handle), (sb))
...
static inline int ext5_handle_has_enough_credits(handle_t *handle, int needed)
{
	if (ext5_handle_valid(handle) && handle->h_buffer_credits < needed)
		return 0;
	return 1;
}

#define ext5_journal_start_sb(sb, type, nblocks)			\
	__ext5_journal_start_sb((sb), __LINE__, (type), (nblocks))

#define ext5_journal_start(inode, type, nblocks)			\
	__ext5_journal_start((inode), __LINE__, (type), (nblocks))

static inline handle_t *__ext5_journal_start(struct inode *inode,
					     unsigned int line, int type,
					     int nblocks)
{
	return __ext5_journal_start_sb(inode->i_sb, line, type, nblocks);
}
...
static inline int ext5_journal_extend(handle_t *handle, int nblocks)
{
	if (ext5_handle_valid(handle))
...
static inline int ext5_journal_blocks_per_page(struct inode *inode)
{
...
static inline int ext5_jbd2_file_inode(handle_t *handle, struct inode *inode)
{
	if (ext5_handle_valid(handle))
		return jbd2_journal_file_inode(handle, EXT5_I(inode)->jinode);
	return 0;
}
...
static inline int ext5_inode_journal_mode(struct inode *inode)
{
	if (EXT5_JOURNAL(inode) == NULL)
	...
	if (ext5_test_inode_flag(inode, EXT5_INODE_JOURNAL_DATA) &&
